...
def print_test_summary(tests_data_keeper: DataKeeperTester, is_ami: bool = True):
    ...
    logging_manager.info('TEST RESULTS:', real_time=True)
    ...
    if skipped_count > 0:
        ...
        logging_manager.warning(''.join([f'\n\t\t\t\t\t\t - {playbook_id}' for playbook_id in skipped_tests]), real_time=True)
